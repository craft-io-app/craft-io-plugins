...
> 4. **Include completed/archived items?** *(Recommended: Yes — historical items are valuable signal)*
...
- Include all statuses: active, done, archived
...
>    - **Child item** — create a dedicated landscape child item under the source item
>    - **Output only** — just show the analysis here

**Ask:**
> 4. **Link items:** Should I create dependencies (type: `related`) between the source item and the located related items via `manage_item_dependency`? *(Select which ones, or say "all", or "none")*
...
### 4A0 — Create Dependencies (if requested)

If the user asked to link items in the question above, call `manage_item_dependency` for each selected item, linking it to the source item with dependency type `related`. Confirm the count and IDs linked before moving on.
...
If the user chose **Child item**: try `create_item`. If permission error, go to 4B.

**Confirm:**
> "I'll create a child item on [source item ID] titled 'Related items landscape: [source title]'. Shall I proceed?"

`create_item`:
- `workspaceId`: same workspace as source item
- `title`: "Related items landscape: [source item title]"
- `type`: story/task-level type per `get_workspace_terminology`
- `parent.id`: source item ID
- `labels`: `["item-landscape", "discovery"]`
- `description`:
...
Analysis complete for [source item ID].
  ...
  [If 4A: Item [ID] created as child of source item.]
...
- Include items from all statuses (active, done, archived) — historical items are valuable signal
